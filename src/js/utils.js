...
window.Eyedom.utils = {
  ...
  writeCache(key, value) {
    try {
      ...
      // Browser storage can be unavailable in private mode or strict settings.
    }
  },

  getMagnitudeColor(magnitude) {
    if (magnitude >= 5) return '#ff2f2f';
    if (magnitude >= 4) return '#ff8a2a';
    if (magnitude >= 3) return '#ffd166';
  ...
  getAlertColor(alertLevel) {
    ...
    if (level.includes('red')) return '#ff2f2f';
    if (level.includes('orange')) return '#ff9f1c';
    if (level.includes('green')) return '#50ff8c';
