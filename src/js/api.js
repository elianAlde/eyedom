...
window.Eyedom.api = (() => {
  ...
  function normalizeDisaster(feature) {
    ...
    return {
      ...
      label: `
        ...
        Allerta: ${utils.escapeHtml(alertLevel || 'n/d')}
      `,
      url: utils.toSafeUrl(
        properties.url ||
        properties.link ||
  ...
  function normalizeNewsArticle(article) {
    return {
      ...
      domain: utils.escapeHtml(article.domain || 'Fonte non indicata'),
  ...
  function normalizeEarthquake(feature) {
    ...
    const place = utils.escapeHtml(properties.place || 'Area non specificata');
    ...
    return {
      ...
      label: `
        ...
        ${place}<br>
        Profondita: ${Math.round(depth)} km
      `
    };
  }
