...
window.Eyedom.ui = (() => {
  ...
  function renderDisasters(items, onSelect) {
    ...
    if (!visibleItems.length) {
      elements.disasterList.innerHTML =
        '<div class="feed-item">Nessuna allerta GDACS recente</div>';
      return;
    }
  ...
  function renderDisasterMeta(item) {
    const details = [
      item.alertLevel ? `Allerta ${item.alertLevel}` : '',
      item.country,
  ...
  function setGlobalNewsLoading() {
    elements.globalNewsList.innerHTML =
      '<div class="feed-item">Caricamento notizie GDELT...</div>';
  }
  ...
  function setEarthquakeError() {
    elements.earthquakeList.innerHTML =
      '<div class="feed-item error">USGS non raggiungibile</div>';
  }

  function setDisasterError() {
    elements.disasterList.innerHTML =
      '<div class="feed-item error">Allerte GDACS non disponibili</div>';
  }

  function setGlobalNewsError() {
    elements.globalNewsList.innerHTML =
      '<div class="feed-item error">GDELT ha limitato le richieste. Riprova tra qualche minuto.</div>';
  }
